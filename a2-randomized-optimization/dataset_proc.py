...
def process_abalone():
    ...
    # transform output into binary classification problem
    df.loc[df['rings'] <= 9, 'rings'] = 0
    df.loc[df['rings'] > 9, 'rings'] = 1

    print(df.groupby('rings', as_index=False).count())
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=HOLDOUT_SIZE, random_state=SEED_VAL)
    ...
    oh_enc = OneHotEncoder()
    y_train = oh_enc.fit_transform(y_train.reshape(-1, 1)).todense()
    y_test = oh_enc.fit_transform(y_test.reshape(-1, 1)).todense()
